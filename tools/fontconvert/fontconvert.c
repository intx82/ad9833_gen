...
int main(int argc, char *argv[]) {
  ...
  printf("from fonts.font import font\n");
  printf("from fonts.font import glyph\n\n");
  printf("class %s(font):\n", fontName);
  printf("    def bitmap(self) -> bytes:\n");
  printf("        return bytearray([\n            ");
  ...
  printf("    def glyph(self, _chr) -> glyph:\n");
  printf("        _glyphs = (\n");
  ...
  printf("    def first(self) -> int:\n");
  printf("        return %d\n\n",first);
  
  printf("    def last(self) -> int:\n");
  printf("        return %d\n\n",last);
  
  printf("    def y_advance(self) -> int:\n");

  if (face->size->metrics.height == 0) {
    // No face height info, assume fixed width and get from a glyph.
    printf("        return %d\n\n",table[0].height);
  } else {
    printf("        return %ld\n\n", face->size->metrics.height >> 7);
  }
